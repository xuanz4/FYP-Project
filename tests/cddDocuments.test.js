...
async function testSaveCddDocumentInsertsRowAndReturnsId() {
  ...
  assert.deepStrictEqual(db.executed[0].params, [
    documentId, 'M-10502', 'Source of Funds', 'bank-statement.pdf', 'abc123.pdf', 'application/pdf', 4096, 'Reviewed June statement', 'USR-001',
  ]);
}
...
async function testSaveCddDocumentDefaultsMissingNotesToNull() {
  ...
  assert.strictEqual(db.executed[0].params[7], null);
}

async function testListCddDocumentsQueriesByMerchantOrderedByRecency() {
  const rows = [{ document_id: 'DOC-1', merchant_id: 'M-1' }];
  const db = fakeDatabase({ queryResult: rows });
  const result = await listCddDocuments(db, 'M-1');
  assert.deepStrictEqual(result, rows);
  assert.match(db.queried[0].sql, /FROM merchant_cdd_documents WHERE merchant_id = \?/);
  assert.match(db.queried[0].sql, /ORDER BY uploaded_at DESC/);
  assert.deepStrictEqual(db.queried[0].params, ['M-1']);
}
...
function testDocumentTypesCoversEveryEddChecklistCategory() {
  assert.deepStrictEqual(DOCUMENT_TYPES, ['Source of Funds', 'Site Visit', 'Enhanced Verification', 'Screening', 'Other']);
}
...
async function main() {
  ...
  await runTest('lists documents for a merchant ordered by recency', testListCddDocumentsQueriesByMerchantOrderedByRecency);
